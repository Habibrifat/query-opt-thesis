...
# --- added: low end (2 tables) and the q10-core shape (worst generalization
# case from evaluate_tpch.py) -> a full 2/3/4/5-table complexity ladder ----
...
def candidates_2table():
    seqs = {"cust-first (c-o)": ["customer", "orders"],
...
def preds_q10(stats):
    # same predicates as evaluate_tpch.py's "q10-core"
    return [
...
SCENARIOS["2table"] = dict(default_tables=TABLES_2, preds_fn=preds_2table,
                           candidates_fn=candidates_2table)
SCENARIOS["q10"] = dict(default_tables=TABLES_4, preds_fn=preds_q10,
                        candidates_fn=candidates_q10)
...
def main():
    ...
    for n in names:
        ...
        rho_pg = spearmanr(pg_costs, times).statistic if len(rows) >= 3 else float("nan")
